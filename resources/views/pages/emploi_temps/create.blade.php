        ...
        <div class="page-header-content pt-4">
            <div class="row align-items-center justify-content-between">
                <div class="col-auto mt-4">
                    ...
                    <div class="page-header-subtitle">Ajouter un créneau à l'emploi du temps</div>
                </div>
                <div class="col-auto mt-4">
        ...
        <div class="col-xl-8">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('gestion_emploi_temps.store') }}">
                        @csrf
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="_classe_id" id="classe_id" class="form-select" onchange="filterEmc(this.value)">
                                    @foreach($classes as $cl)
                                        <option value="{{ $cl->id }}" @selected($cl->id == $classeId)>{{ $cl->nom }}</option>
                                    @endforeach
                                    @if($classes->isEmpty())
                                        <option disabled>Aucune classe pour cette année</option>
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="col-12">
                                <label class="form-label fw-500">Matière — Enseignant <span class="text-danger">*</span></label>
                                ...
                                @error('enseignement_matiere_classe_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <!-- Jour -->
                            <div class="col-md-4">
                                <label class="form-label fw-500">Jour <span class="text-danger">*</span></label>
                                <select name="jour_semaine" class="form-select @error('jour_semaine') is-invalid @enderror">
                                    <option value="">— Choisir —</option>
                                    @foreach(\App\Models\EmploiTemp::JOURS as $j)
                                        <option value="{{ $j }}" @selected(old('jour_semaine') == $j)>{{ $j }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="time" name="heure_fin" value="{{ old('heure_fin', '09:00') }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" name="salle" value="{{ old('salle') }}"
                                       class="form-control @error('salle') is-invalid @enderror" placeholder="Ex: Salle 12">
...
const selectedEmc = {{ old('enseignement_matiere_classe_id', 0) }};
...
function filterEmc(classeId) {
    ...
    items.forEach(emc => {
        ...
        opt.value = emc.id;
        opt.text  = (emc.matiere?.intitule || 'Matière?') + ' — ' + (emc.enseignant?.prenom || '') + ' ' + (emc.enseignant?.nom || '');
...
function reloadPage(anneeId, classeId) {
    let url = "{{ route('gestion_emploi_temps.create') }}?annee_id=" + anneeId;
